...
nonzero_reward_rows = (df["reward"] != 0).sum()

valid_reward_rows = df["reward"].notna().sum()
...
print(f"Rows with valid rewards: {valid_reward_rows}")
print(f"Valid reward fraction: {valid_reward_rows / total_rows:.4f}")

print(f"Rows with nonzero rewards: {nonzero_reward_rows}")
print(f"Nonzero reward fraction: {nonzero_reward_rows / total_rows:.4f}")

print("\nReward statistics:")
print(df["reward"].describe())

print("\nReward percentiles:")
print(df["reward"].quantile([0, 0.01, 0.05, 0.25, 0.5, 0.75, 0.95, 0.99, 1]))

print("\nAction distribution:")
print(df["action_name"].value_counts(normalize=True))

print("\nObserved mood by action:")
print(df.groupby("action_name")["mood_observed"].mean())

print("\nAverage reward by action:")
print(df.groupby("action_name")["reward"].mean())
