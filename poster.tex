...
\justifying
All numbers from chronological \textbf{test} split (\texttt{models/extended\_comparison\_results.csv}
and \texttt{dqn\_saved\_model\_ope\_metrics.csv}).
Sequential OPE uses behavior cloning as logging policy (IS clip $= 5$).
WPDIS = self-normalized PDIS (primary offline metric).
DR = sequential doubly robust (Q-policies); $^\dagger$ = bandit one-step DR.
$\Delta$mood = matched-action, mood-observed steps; $\mathrm{NA}\,(0)$ = no valid mood pairs.
...
\justifying
\footnotesize
\textit{Notes.} $^\dagger$ = one-step bandit DR (not sequential).
CQL DR unstable; treat as diagnostic only.
$\Delta$mood counts in parentheses; values with $n<5$ are unreliable.
ESS $\approx 2$--$2.5$ episodes for DQN variants limits IS reliability.
Regenerate rows: \texttt{python algorithms/visualize\_policy\_comparison.py --export-latex}.
\end{block}
...
\footnotesize
\justifying
\textit{Two-panel plot: PDIS / WPDIS (top) and DR (bottom) with 95\% bootstrap
error bars (episode resampling). DR panel clipped for readability; full CIs in
table. Focus policies: Behavior Cloning, Ctx.~Bandit, BCQ, CQL, DQN, Double DQN.}
\end{block}
